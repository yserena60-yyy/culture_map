...
-- 1. Ensure wikidata_entity_id exists in places
...
-- 3. FORCE ADD missing columns in case the tables already existed but were missing them!
ALTER TABLE public.landmark_ratings
  ADD COLUMN IF NOT EXISTS rating INTEGER CHECK (rating >= 1 AND rating <= 5),
  ADD COLUMN IF NOT EXISTS comment TEXT;

ALTER TABLE public.landmark_stories
  ADD COLUMN IF NOT EXISTS content TEXT;

-- 4. Fix RLS for places so users can actually insert!
...
-- 6. Recreate the view (now it will succeed because r.rating is guaranteed to exist)
DROP VIEW IF EXISTS public.landmark_stats;
CREATE VIEW public.landmark_stats AS
SELECT
  p.id                              AS place_id,
